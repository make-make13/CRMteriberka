...
assert.match(source, /EXECUTOR_SIGNATURE_BLOCK_PX\s*=\s*\{\s*width:\s*190,\s*height:\s*80\s*\}/, 'executor signature block size must stay compact.');
assert.match(source, /EXECUTOR_SIGNATURE_BLOCK_LINE_TWP\s*=\s*26\s*\*\s*15/, 'guest line alignment must follow the line position inside the fixed executor block.');
