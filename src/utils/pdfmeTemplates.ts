...
function normalizeChungaChangaContractPage(page: Template['schemas'][number]) {
  const hasBmField = page.some(schema => String((schema as Record<string, unknown>).name || '').startsWith('bm_'));

  if (hasBmField) {
    return migrateChungaChangaContractPage(page).map(schema => {
      const next = { ...(schema as Record<string, unknown>) };
      const name = String(next.name || '');

      // Ensure correct fontName
      ...
      if (next.type === 'multiVariableText') {
        next.boldVariableFontName = 'NotoSerifBold';
      }

      // Populate preview values for multiVariableText in template preview mode
...
function createCcContractPage1Schemas() {
  return [
    ...
    mvt(
      'cc_client_intro',
      'ООО «Золото Арктики», осуществляющее деятельность под коммерческим обозначением бутик-отель «Большая Медведица», в лице'
      + ' генерального директора Сташ Екатерины Александровны, действующего на основании Устава, именуемое в дальнейшем'
      + ' «Исполнитель», с одной стороны, и {client_name}, именуемый в дальнейшем «Гость», с другой стороны, заключили'
    ...
    mvt(
      'bm_s2_body',
      '2.1. Исполнитель обязуется оказать Гостю гостиничные услуги по временному размещению в бутик-отеле «Большая Медведица»,'
      + ' а Гость обязуется принять и оплатить указанные услуги в порядке, размере и на условиях, предусмотренных настоящим договором,'
      + ' подтверждением бронирования, выбранным тарифом и Правилами проживания бутик-отеля.\n'
      + '2.2. В рамках настоящего договора Исполнитель предоставляет Гостю номер №{cottage_number} категории «{room_category}» для'
      + ' временного проживания {guests} человека. Размещение предоставляется на период с {time_in} {date_in_short} по {time_out} {date_out_short}.'
      + ' Срок проживания составляет {nights_label}.\n'
      + '2.3. Размещение допускается только для лиц, указанных в бронировании, подтверждении бронирования и/или настоящем договоре.'
      + ' Передача ключей или карты доступа третьим лицам, проживание незарегистрированных лиц и нахождение посторонних лиц в номере без'
      + ' оформления у администратора не допускаются. Гость несёт ответственность за действия лиц, которым он предоставил доступ в номер или'
      + ' на территорию бутик-отеля. Нарушение указанного порядка является нарушением настоящего договора и Правил проживания бутик-отеля.',
      ['cottage_number', 'room_category', 'guests', 'time_in', 'date_in_short', 'time_out', 'date_out_short', 'nights_label'],
      20, 105, 173, 31,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
    ...
    st(
      ...
      + '3.2. Если иное не указано в подтверждении бронирования, в базовый тариф могут входить: временное размещение в номере'
      + ' выбранной категории, завтрак при включении в тариф, Wi-Fi на территории, плановая уборка номера, пользование мебелью,'
      + ' оборудованием и текстилем по назначению, доступные общественные зоны бутик-отеля.\n'
      + '3.3. Wi-Fi предоставляется без гарантии постоянной скорости и непрерывности соединения. Парковка'
      + ' предоставляется при наличии свободных мест, без закрепления конкретного места и без принятия ТС на хранение.\n'
      + '3.4. Дополнительные услуги и принадлежности предоставляются при наличии технической и организационной'
      + ' возможности — бесплатно, во временное пользование или за дополнительную плату согласно прейскуранту.\n'
      + '3.5. Помощь администратора с вызовом такси, трансфера, экскурсий, справочной информацией является'
    ...
    mvt(
      'bm_s4_body',
      '4.1. Общая стоимость проживания по настоящему договору составляет: {total} рублей ({total_words}).\n'
      + '4.2. Для гарантированного бронирования Гость вносит предоплату в размере стоимости первых суток проживания,'
      + ' если иной размер предоплаты не указан в подтверждении бронирования, счёте или выбранном тарифе.\n'
      + '4.3. Размер предоплаты составляет: {prepayment} рублей ({prepayment_words}).\n'
      + '4.4. Оставшаяся часть стоимости проживания составляет: {balance} рублей ({balance_words}) и оплачивается Гостем'
      + ' не позднее момента заселения, если иной срок оплаты не указан в подтверждении бронирования, счёте или выбранном тарифе.\n'
      + '4.5. Оплата может производиться банковским переводом на расчётный счёт Исполнителя, банковской картой,'
      + ' наличными денежными средствами в кассу бутик-отеля либо иным способом, согласованным с Исполнителем.\n'
      + '4.6. Моментом оплаты считается момент фактического поступления денежных средств Исполнителю или'
      + ' подтверждения успешной операции платежной системой.\n'
      + '4.7. Дополнительные услуги, причинённый ущерб, утрата ключей или карты доступа, расходы на специальную'
      + ' уборку и иные начисления, возникшие вследствие нарушения настоящего договора или Правил проживания'
      + ' бутик-отеля, оплачиваются Гостем отдельно на основании акта, расчёта, фото- или видеофиксации при наличии'
      + ' и/или утверждённого прейскуранта Исполнителя.\n'
      + '4.8. Отсутствие предварительного удержания денежных средств за возможный ущерб не освобождает Гостя от'
      + ' обязанности оплатить оказанные дополнительные услуги, возместить причинённый ущерб, расходы на'
      + ' восстановление имущества, специальную уборку,',
...
function createCcContractPage2Schemas() {
  return [
    ...
    st(
      ...
      + '5.2. Подтверждение бронирования, уведомления, счета, сообщения об изменении, отмене или уточнении условий'
      ...
      + '5.3. Конкретные условия отмены, переноса и возврата определяются выбранным тарифом, указанным в'
      + ' подтверждении бронирования, с учетом требований законодательства Российской Федерации.\n'
      + '5.4. При гарантированном бронировании Исполнитель ожидает Гостя до расчётного часа дня, следующего за'
      + ' днем запланированного заезда. После указанного времени при незаезде Гостя Исполнитель вправе отказаться'
      + ' от исполнения договора.\n'
      + '5.5. Если Гость уведомил Исполнителя об отказе от бронирования до дня заезда, внесенная плата'
      ...
      + '5.6. Если Гость уведомил Исполнителя об отказе от бронирования в день заезда, после наступления дня'
      + ' заезда, nesvoevremenno, опоздал либо не заехал, Исполнитель вправе удержать плату за простой'
      + ' номера, но не более стоимости одних суток проживания.\n'
      + '5.7. Возврат денежных средств осуществляется тем же способом, которым была произведена оплата, либо иным'
      + ' согласованным Сторонами способом в сроки, предусмотренные законодательством РФ и правилами платежных систем.\n'
      + '5.8. При досрочном выезде Гость оплачивает фактически оказанные услуги, а также фактически понесенные'
      + ' Исполнителем расходы, связанные с исполнением договора. Возврат за неиспользованный период проживания'
      + ' производится после перерасчёта.',
      20, 22, 173, 56,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
    ...
    st(
      ...
      + '6.2. Заселение производится при предъявлении оригинала документа, удостоверяющего личность, необходимого'
      ...
      + '6.3. Исполнитель вправе отказать в заселении лицу, не предъявившему документ, удостоверяющий личность,'
      ...
      + '6.4. Ранний заезд и поздний выезд не являются обязательными услугами и предоставляются только при наличии'
      ...
      + '6.5. При выезде Гость обязан освободить номер, вернуть ключи/карту доступа, сдать номер администратору'
      + ' и оплатить все оказанные дополнительные услуги, причинённый ущерб и иные начисления при их наличии.\n'
      + '6.6. При задержке выезда без согласования с администрацией Исполнитель вправе начислить плату за поздний'
    ...
    st(
      ...
      + '7.2. В номерах и внутренних помещениях бутик-отеля запрещены курение табачных изделий, потребление'
      + ' никотинсодержащей продукции, использование электронных сигарет, вейпов, кальянов и иных аналогичных устройств.\n'
      + '7.3. При нарушении запрета на курение Гость возмещает Исполнителю фактически понесенные расходы, включая'
      ...
      + '7.4. Запрещается разводить открытый огонь, использовать пиротехнические изделия, свечи, хлопушки с'
      ...
      + '7.5. Размещение, нахождение и пребывание гостей с любыми животными на территории бутик-отеля и в номерах'
      ...
      + '7.6. Гость не вправе использовать номер для целей, не связанных с временным проживанием, в том числе для'
      ...
      + '7.7. Ночной режим тишины, порядок пользования общими зонами, правила посещения гостей и иные внутренние'
      + ' требования определяются Правилами проживания бутик-отеля.',
      20, 144, 173, 59,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §8 Имущество бутик-отеля и ответственность Гостя (part 1)
    st('bm_s8_title', '8. Имущество бутик-отеля и ответственность Гостя', 20, 206, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s8_body_part1',
...
function createCcContractPage3Schemas() {
  return [
    st(
      'bm_s8_body_part2',
      '8.2. Гость несёт ответственность за утрату, повреждение или загрязнение имущества бутик-отеля по своей'
      + ' вине, а также по вине приглашённых им лиц, несовершеннолетних лиц, за которых он отвечает, и иных лиц,'
      + ' которым он предоставил доступ в номер.\n'
      + '8.3. Размер ущерба определяется на основании акта, фото- и видеоматериалов при их наличии, объяснений'
      + ' сторон, стоимости восстановления, чистки, ремонта, замены имущества и действующего прейскуранта Исполнителя.\n'
      + '8.4. Отказ Гостя от подписания акта не освобождает его от обязанности возместить ущерб. В таком случае'
      + ' Исполнитель вправе составить односторонний акт с участием не менее двух представителей Исполнителя либо с'
      + ' использованием фото- и видеофиксации.\n'
      + '8.5. При утрате ключа/карты доступа, повреждении замка, мебели, техники, сантехники, текстиля, посуды или'
      + ' иного имущества Гость возмещает стоимость восстановления или замены по фактическим расходам Исполнителя'
      + ' либо по утвержденному прейскуранту.',
      20, 8, 173, 30,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §9 Ответственность Исполнителя и хранение вещей
    st('bm_s9_title', '9. Ответственность Исполнителя и хранение вещей', 20, 41, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s9_body',
      ...
      + '9.2. Исполнитель отвечает за сохранность вещей Гостя в случаях и пределах, предусмотренных законодательством РФ.\n'
      + '9.3. Денежные средства, документы, ювелирные изделия, электронная техника и иные ценные вещи рекомендуется'
      + ' сдавать администратору на хранение, если такая услуга доступна, либо хранить с принятием разумных мер предосторожности.\n'
      + '9.4. О пропаже, повреждении или утрате вещей Гость обязан незамедлительно сообщить администратору для фиксации'
      + ' обстоятельств, составления акта и проверки доступных материалов, включая записи камер при их наличии.\n'
      + '9.5. Исполнитель не отвечает за убытки Гостя, возникшие вследствие нарушения Гостем договора, Правил проживания,'
      + ' требований безопасности, правил пользования имуществом, а также вследствие действий третьих лиц, если ответственность'
      + ' Исполнителя не установлена законодательством РФ.\n'
      + '9.6. Исполнитель не несет ответственность за временное отсутствие или перебои в работе внешних коммунальных,'
      + ' телекоммуникационных, транспортных и иных служб, если такие перебои возникли не по вине Исполнителя. При этом'
      + ' Исполнитель принимает разумные меры для уменьшения неудобств Гостя в пределах технической и организационной возможности.\n'
      + '9.7. Исполнитель не является перевозчиком, туроператором, транспортной компанией или службой трансфера, если'
      + ' соответствующая услуга не оформлена отдельным договором или заказом. Информация о погоде, дороге, трансфере, такси,'
      + ' экскурсиях и маршрутах предоставляется как справочная поддержка.',
      20, 47, 173, 59,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §10 Односторонний отказ и прекращение проживания
    st('bm_s10_title', '10. Односторонний отказ и прекращение проживания', 20, 109, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s10_body',
      ...
      + '10.2. Исполнитель вправе отказаться от исполнения договора в случаях и порядке, предусмотренных законодательством РФ.\n'
      + '10.3. При грубом нарушении Гостем Правил проживания, требований пожарной безопасности, общественного порядка, прав'
      + ' других гостей, создании угрозы жизни, здоровью или имуществу, а также при отказе прекратить нарушение после'
      + ' требования администрации Исполнитель вправе потребовать прекращения нарушения, возмещения ущерба и освобождения'
      + ' номера в порядке, предусмотренном законодательством РФ.\n'
      + '10.4. Прекращение проживания по причинам, связанным с виновными действиями Гостя, не освобождает Гостя от оплаты'
      + ' фактически оказанных услуг, фактически понесённых Исполнителем расходов и возмещения причинённого ущерба.',
      20, 115, 173, 34,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §11 Персональные данные и электронные коммуникации
    st('bm_s11_title', '11. Персональные данные и электронные коммуникации', 20, 152, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s11_body',
      ...
      + ' регистрации проживания, бухгалтерского учета, исполнения требований законодательства РФ и связи с Гостем по вопросам проживания.\n'
      + '11.2. Исполнитель обрабатывает персональные данные Гостя в объеме и целях, необходимых для исполнения договора и'
      + ' требований законодательства РФ.\n'
      + '11.3. Юридически значимые уведомления, подтверждения бронирования, счета, сообщения об изменении или отмене'
      + ' бронирования могут направляться по адресу электронной почты, номеру телефона, через сайт, систему бронирования'
      + ' или иной канал связи, указанный Гостем при бронировании.\n'
      + '11.4. Стороны признают переписку, подтверждения бронирования, счета, платежные документы, электронные сообщения'
      + ' и иные материалы, позволяющие установить отправителя, содержание и дату сообщения, допустимыми доказательствами'
      + ' условий бронирования, оплаты и исполнения договора.',
      20, 158, 173, 31,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §12 Форс-мажор, погодные условия и транспортная доступность (part 1)
    st('bm_s12_title', '12. Форс-мажор, погодные условия и транспортная доступность', 20, 192, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s12_body_part1',
      ...
      + ' автомобильных дорог, переметы, штормовые предупреждения, действия органов власти и иные обстоятельства вне'
      + ' разумного контроля Сторон.\n'
      + '12.2. Сторона, для которой создалась невозможность исполнения обязательств, уведомляет другую Сторону в'
      + ' разумный срок и принимает зависящие от нее меры для уменьшения возможных последствий.\n'
      + '12.3. Гость уведомлен, что бутик-отель расположен в с. Териберка, где транспортная доступность может зависеть'
      + ' от погодных условий, состояния автомобильных дорог, работы внешних транспортных служб, ограничений движения и'
      + ' решений уполномоченных органов.\n'
      + '12.4. Исполнитель не гарантирует возможность прибытия Гостя в бутик-отель или обратного выезда Гостя из с.'
      + ' Териберка в конкретную дату и время, если препятствия возникли вследствие погодных условий, ограничений проезда,'
      + ' перекрытия дорог, отсутствия транспорта, действий третьих лиц или обстоятельств, не зависящих от Исполнителя.\n'
      + '12.5. Расходы Гостя, связанные с изменением маршрута, ожиданием транспорта, продлением поездки, изменением'
      + ' авиабилетов, трансфера, экскурсий или иных услуг третьих лиц, не возмещаются Исполнителем, если такие расходы'
      + ' возникли не по вине Исполнителя.\n'
      + '12.6. Перекрытие дороги, неблагоприятные погодные условия, отсутствие транспортного сообщения или невозможность'
      + ' Гостя своевременно покинуть бутик-отель не являются основанием для бесплатного продления проживания, если иное'
      + ' отдельно не',
      20, 198, 173, 56,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
...
function createCcContractPage4Schemas() {
  return [
    ...
    st(
      ...
      + '13.3. При невозможности урегулировать спор путем переговоров спор подлежит рассмотрению в судебном порядке в'
    ...
    st(
      ...
      + ' зависимости от того, какое событие наступило раньше.\n'
      + '14.2. Договор действует до полного исполнения Сторонами обязательств, включая оплату услуг, возврат ключей,'
      + ' возмещение ущерба при его наличии и проведение окончательных расчетов.\n'
      + '14.3. Правила проживания бутик-отеля, подтверждение бронирования, действующий тариф, счет, прейскурант'
      ...
      + '14.4. Если отдельное положение договора будет признано недействительным, это не влияет на действительность'
      + ' остальных положений договора.\n'
      + '14.5. Гость подтверждает, что до заключения договора получил необходимую информацию об Исполнителе, бутик-отеле,'
      + ' условиях проживания, цене, порядке оплаты, условиях бронирования и отмены, Правилах проживания бутик-отеля,'
      + ' а также о правах, обязанностях и ограничениях, связанных с проживанием.\n'
      + '14.6. Подписание настоящего договора Гостем означает согласие Гостя с Правилами проживания бутик-отеля «Большая'
      + ' Медведица». Гость подтверждает, что обязуется соблюдать Правила проживания лично, а также обеспечить их'
      + ' соблюдение зарегистрированными гостями, несовершеннолетними лицами, сопровождаемыми Гостем, посетителями и'
      + ' иными лицами, допущенными Гостем на территорию бутик-отеля или в номер.',
      20, 56, 173, 51,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
    ...
    imageField('cc_executor_signature_image', 29, 194, 34, 20),
    imageField('cc_company_stamp_image', 21, 186, 44, 44, { opacity: 0.68 }),
    l('cc_executor_sign_line', 44, 214, 37, 0.1),
    st('cc_executor_sign_name', '/Е. А. Сташ/', 74, 210, 24, 5, { fontSize: 7.8 }),
