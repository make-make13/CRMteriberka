...
function createGbContractPage1Schemas() {
  return [
...
function createGbContractPage2Schemas() {
  return [
    st(
      'cc_section_3_tail',
...
function createDefaultGolubayaBukhtaContractOnlyPdfmeTemplate(): Template {
  return {
    basePdf: { width: 210, height: 297, padding: [5, 15, 5, 20] },
    schemas: [
      applyStandardLayoutOverrides(createGbContractPage1Schemas()),
      createGbContractPage2Schemas(),
    ],
  };
}
...
function createCcContractPage1Schemas() {
  return [
    mvt('cc_contract_header', 'ДОГОВОР №{contract_number}', ['contract_number'], 64, 8, 82, 7, {
      fontName: 'NotoSerifBold',
      fontSize: 9.8,
      alignment: 'center',
      verticalAlignment: 'middle',
    }),
    st('cc_contract_subtitle', 'оказания гостиничных услуг / подтверждение бронирования', 55, 17.2, 100, 7, {
      alignment: 'center',
    }),
    st('cc_contract_city', 'с. Териберка, Мурманская обл.', 20, 31, 86, 5, { fontSize: 7.9 }),
    ...
    mvt(
      'cc_client_intro',
      'Общество с ограниченной ответственностью «Золото Арктики», ОГРН 1215100000158, ИНН 5105013870, КПП 510501001,'
      + ' осуществляющее деятельность под коммерческим обозначением бутик-отель «Большая Медведица», именуемое'
      + ' в дальнейшем «Исполнитель», в лице генерального директора Сташ Екатерины Александровны, действующего на'
      + ' основании Устава, с одной стороны, и {client_name}, документ, удостоверяющий личность: {client_passport},'
      + ' именуемый(ая) в дальнейшем «Гость», с другой стороны, заключили настоящий договор о нижеследующем.',
    ...
    st(
      ...
      + ' предоплаты либо фактическом заселении Гостя.\n'
      + '1.2. Подтверждение бронирования, выбранный тариф, Правила проживания бутик-отеля, прейскурант дополнительных'
      + ' услуг и компенсаций, а также иные документы, связанные с бронированием, размещением, оплатой услуг,'
      + ' фиксацией ущерба или оказанием дополнительных услуг, применяются совместно с настоящим договором.\n'
      + '1.3. При противоречии документов применяется следующий приоритет: настоящий договор; подтверждение'
      + ' бронирования и условия тарифа; Правила проживания бутик-отеля; прейскурант; иные информационные материалы.\n'
      + '1.4. Оплата, предоплата, подписание настоящего договора или фактическое заселение означает, что Гость'
      + ' ознакомлен с условиями договора, тарифа, подтверждением бронирования, Правилами проживания и условиями'
      + ' отмены бронирования, принимает их и обязуется соблюдать.',
      20, 66, 173, 30,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §2 Сведения об Исполнителе и средстве размещения
    st('bm_s2_title', '2. Сведения об Исполнителе и средстве размещения', 20, 99, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s2_body',
      'Исполнитель: ООО «Золото Арктики»\n'
      + 'Коммерческое обозначение: бутик-отель «Большая Медведица»\n'
      + 'ОГРН / ИНН / КПП: 1215100000158 / 5105013870 / 510501001\n'
      + 'Юридический адрес: 184433, Мурманская обл., Печенгский р-н, г. Заполярный, ул. Ленина, д. 1А, пом. 34\n'
      + 'Фактический адрес бутик-отеля: Мурманская обл., Кольский р-н, с. Териберка, ул. 1-я Пятилетка, д. 1\n'
      + 'Телефон администратора: +7 931 802-21-51\n'
      + 'E-mail: medvedica.hotel@vk.com   Сайт: medvedica-hotel.ru\n'
      + 'Режим работы администратора: круглосуточно\n'
      + 'Уникальный номер реестровой записи средства размещения: C512026024350\n'
      + 'Категория средства размещения: без звезд',
      20, 105, 173, 31,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §3 Предмет договора
    st('bm_s3_title', '3. Предмет договора', 20, 139, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    mvt(
      'bm_s3_body',
      '3.1. Исполнитель обязуется предоставить Гостю гостиничные услуги по временному размещению в бутик-отеле'
      + ' «Большая Медведица», а Гость обязуется принять и оплатить услуги на условиях настоящего договора,'
      + ' подтверждения бронирования и выбранного тарифа.\n'
      + '3.2. Гостю предоставляется номер / место размещения:\n'
      + 'Категория номера: {room_category}     Номер: {cottage_number}\n'
      + 'Ф.И.О. Гостя: {client_name}\n'
      + 'Количество проживающих: {guests} чел.\n'
      + 'Дата и время заезда: {date_in_short}, с {time_in} местного времени\n'
      + 'Дата и время выезда: {date_out_short}, до {time_out} местного времени\n'
      + 'Количество суток проживания: {nights_label}\n'
      + '3.3. Размещение допускается только для лиц, указанных в бронировании и/или договоре. Передача ключей или'
      + ' карты доступа третьим лицам, проживание незарегистрированных лиц и нахождение посторонних в номере без'
      + ' оформления у администратора не допускаются.',
      ['room_category', 'cottage_number', 'client_name', 'guests', 'date_in_short', 'time_in', 'date_out_short', 'time_out', 'nights', 'nights_label'],
      20, 145, 173, 39,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §4 Состав услуг
    st('bm_s4_title', '4. Состав услуг', 20, 187, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s4_body',
      '4.1. В стоимость проживания входит набор услуг, указанный в подтверждении бронирования, выбранном тарифе,'
      + ' настоящем договоре или Правилах проживания бутик-отеля. Бесплатные услуги, предусмотренные законодательством'
      + ' РФ и Правилами проживания бутик-отеля, предоставляются при наличии соответствующих условий и технической возможности.\n'
      + '4.2. Если иное не указано в подтверждении бронирования, в базовый тариф могут входить: временное размещение в'
      + ' номере выбранной категории, завтрак при включении в тариф, Wi-Fi на территории, плановая уборка номера,'
      + ' пользование мебелью, оборудованием и текстилем по назначению, доступные общественные зоны бутик-отеля.\n'
      + '4.3. Wi-Fi предоставляется без гарантии постоянной скорости и непрерывности соединения. Парковка'
      + ' предоставляется при наличии свободных мест, без закрепления конкретного места и без принятия ТС на хранение.\n'
      + '4.4. Дополнительные услуги и принадлежности предоставляются при наличии технической и организационной'
      + ' возможности — бесплатно, во временное пользование или за дополнительную плату согласно прейскуранту.\n'
      + '4.5. Помощь администратора с вызовом такси, трансфера, экскурсий, справочной информацией является'
      + ' информационной поддержкой и не означает, что Исполнитель оказывает транспортные, экскурсионные или иные'
      + ' самостоятельные услуги, если это прямо не оформлено отдельным договором или заказом.',
      20, 193, 173, 39,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
  ];
...
function createCcContractPage2Schemas() {
  return [
    ...
    st('bm_s5_title', '5. Цена и порядок оплаты', 20, 8, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    mvt(
      'bm_s5_body',
      '5.1. Общая стоимость проживания по настоящему договору составляет: {total} рублей ({total_words}).\n'
      + '5.2. Для гарантированного бронирования Гость вносит предоплату в размере стоимости первых суток проживания,'
      + ' если иной размер предоплаты не указан в подтверждении бронирования, счете или выбранном тарифе.\n'
      + '5.3. Размер предоплаты составляет: {prepayment} рублей ({prepayment_words}).\n'
      + '5.4. Оставшаяся часть стоимости проживания составляет: {balance} рублей ({balance_words}) и оплачивается'
      + ' Гостем не позднее момента заселения, если иной срок оплаты не указан в подтверждении бронирования,'
      + ' счете или выбранном тарифе.\n'
      + '5.5. Оплата может производиться банковским переводом на расчетный счет Исполнителя, банковской картой,'
      + ' наличными денежными средствами в кассу бутик-отеля либо иным способом, согласованным с Исполнителем.\n'
      + '5.6. Моментом оплаты считается момент фактического поступления денежных средств Исполнителю или'
      + ' подтверждения успешной операции платежной системой.\n'
      + '5.7. Дополнительные услуги, причиненный ущерб, утрата ключей или карты доступа, расходы на специальную'
      + ' уборку и иные начисления, возникшие вследствие нарушения настоящего договора или Правил проживания'
      + ' бутик-отеля, оплачиваются Гостем отдельно на основании акта, расчета, фото- или видеофиксации при'
      + ' наличии и/или утвержденного прейскуранта Исполнителя.\n'
      + '5.8. Отсутствие предварительного удержания денежных средств за возможный ущерб не освобождает Гостя от'
      + ' обязанности оплатить оказанные дополнительные услуги, возместить причиненный ущерб, расходы на'
      + ' восстановление имущества, специальную уборку, простой номера и иные документально подтвержденные расходы'
      + ' Исполнителя, возникшие по вине Гостя или лиц, которым Гость предоставил доступ в номер.',
      ['total', 'total_words', 'prepayment', 'prepayment_words', 'balance', 'balance_words'],
      20, 14, 173, 56,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §6 Бронирование, отмена, незаезд и досрочный выезд
    st('bm_s6_title', '6. Бронирование, отмена, незаезд и досрочный выезд', 20, 73, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s6_body',
      '6.1. Бронирование считается гарантированным после получения Исполнителем установленной предоплаты и'
      + ' направления Гостю подтверждения бронирования.\n'
      + '6.2. Подтверждение бронирования, уведомления, счета, сообщения об изменении, отмене или уточнении условий'
      + ' бронирования могут быть направлены Гостю по электронной почте, SMS, через сайт, систему бронирования,'
      + ' мессенджер, телефон, иной официальный канал связи, указанный Гостем или использованный им при'
      + ' бронировании, а также вручены Гостю лично. Исполнитель вправе использовать любой доступный контактный'
      + ' способ связи с Гостем по вопросам бронирования, оплаты, заселения, проживания или выезда.\n'
      + '6.3. Конкретные условия отмены, переноса и возврата определяются выбранным тарифом, указанным в'
      + ' подтверждении бронирования, с учетом требований законодательства Российской Федерации.\n'
      + '6.4. При гарантированном бронировании Исполнитель ожидает Гостя до расчетного часа дня, следующего за'
      + ' днем запланированного заезда. После указанного времени при незаезде Гостя Исполнитель вправе отказаться'
      + ' от исполнения договора.\n'
      + '6.5. Если Гость уведомил Исполнителя об отказе от бронирования до дня заезда, внесенная плата'
      + ' возвращается Гостю в полном размере, если иные условия не установлены специальным тарифом, акцией,'
      + ' договором, подтверждением бронирования или правилами системы онлайн-бронирования и не противоречат'
      + ' законодательству Российской Федерации.\n'
      + '6.6. Если Гость уведомил Исполнителя об отказе от бронирования в день заезда, после наступления дня'
      + ' заезда, несвоевременно, опоздал либо не заехал, Исполнитель вправе удержать плату за фактический простой'
      + ' номера, но не более стоимости одних суток проживания.\n'
      + '6.7. Возврат денежных средств осуществляется тем же способом, которым была произведена оплата, либо иным'
      + ' согласованным Сторонами способом в сроки, предусмотренные законодательством РФ и правилами платежных систем.\n'
      + '6.8. При досрочном выезде Гость оплачивает фактически оказанные услуги, а также фактически понесенные'
      + ' Исполнителем расходы, связанные с исполнением договора. Возврат за неиспользованный период проживания'
      + ' производится после перерасчета.',
      20, 79, 173, 56,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §7 Заселение, регистрация и выезд
    st('bm_s7_title', '7. Заселение, регистрация и выезд', 20, 138, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s7_body',
      '7.1. Заезд осуществляется с 14:00 местного времени, выезд — до 12:00 местного времени, если иное не указано'
      + ' в подтверждении бронирования.\n'
      + '7.2. Заселение производится при предъявлении оригинала документа, удостоверяющего личность, необходимого'
      + ' для регистрации Гостя в соответствии с законодательством Российской Федерации. Заселение'
      + ' несовершеннолетних, иностранных граждан и лиц без гражданства осуществляется при предъявлении документов'
      + ' и сведений, предусмотренных законодательством РФ и Правилами проживания бутик-отеля.\n'
      + '7.3. Исполнитель вправе отказать в заселении лицу, не предъявившему документ, удостоверяющий личность,'
      + ' лицу, не указанному в бронировании, лицу, не оплатившему стоимость проживания, а также при превышении'
      + ' допустимой вместимости номера.\n'
      + '7.4. Ранний заезд и поздний выезд не являются обязательными услугами и предоставляются только при наличии'
      + ' свободного подготовленного номера, технической и организационной возможности бутик-отеля и'
      + ' предварительном согласовании с администрацией. Ранний заезд, поздний выезд, заселение до установленного'
      ...
      + '7.5. При выезде Гость обязан освободить номер, вернуть ключи / карту доступа, сдать номер администратору'
      + ' и оплатить все оказанные дополнительные услуги, причиненный ущерб и иные начисления при их наличии.\n'
      + '7.6. При задержке выезда без согласования с администрацией Исполнитель вправе начислить плату за поздний'
      + ' выезд согласно действующему тарифу, а также потребовать освобождения номера.',
      20, 144, 173, 48,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
  ];
...
function createCcContractPage3Schemas() {
  return [
    // §8 Правила проживания и ограничения
    st('bm_s8_title', '8. Правила проживания и ограничения', 20, 8, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s8_body',
      '8.1. Гость обязан соблюдать Правила проживания бутик-отеля, требования пожарной безопасности, санитарные'
      + ' нормы, общественный порядок, права других гостей и законные требования администрации.\n'
      + '8.2. В номерах и внутренних помещениях бутик-отеля запрещены курение табачных изделий, потребление'
      + ' никотинсодержащей продукции, использование электронных сигарет, вейпов, кальянов и иных аналогичных устройств.\n'
      + '8.3. При нарушении запрета на курение Гость возмещает Исполнителю фактически понесенные расходы, включая'
      + ' расходы на специальную уборку, санитарную обработку, стирку или замену текстиля, устранение запаха и'
      + ' простой номера. Минимальная расчетная стоимость таких расходов может устанавливаться действующим'
      + ' прейскурантом Исполнителя.\n'
      + '8.4. Запрещается разводить открытый огонь, использовать пиротехнические изделия, свечи, хлопушки с'
      + ' конфетти, легковоспламеняющиеся вещества, а также совершать действия, создающие угрозу жизни, здоровью'
      + ' гостей, персоналу или имуществу бутик-отеля.\n'
      + '8.5. Размещение, нахождение и пребывание гостей с любыми животными на территории бутик-отеля и в номерах'
      + ' полностью запрещено. Запрет распространяется на собак, кошек, птиц, грызунов, рептилий, декоративных,'
      + ' экзотических и иных животных независимо от породы, размера, веса, возраста, наличия переноски, клетки или'
      + ' контейнера. Исключения допускаются только в случаях, когда бутик-отель обязан допустить животное в силу'
      + ' прямого требования законодательства Российской Федерации.\n'
      + '8.6. Гость не вправе использовать номер для целей, не связанных с временным проживанием, в том числе для'
      + ' проведения мероприятий, коммерческой деятельности, фото- и видеосъемок коммерческого характера без'
      + ' предварительного письменного согласия Исполнителя.\n'
      + '8.7. Ночной режим тишины, порядок пользования общими зонами, правила посещения гостей и иные внутренние'
      + ' требования определяются Правилами проживания бутик-отеля.',
      20, 14, 173, 59,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §9 Имущество бутик-отеля и ответственность Гостя
    st('bm_s9_title', '9. Имущество бутик-отеля и ответственность Гостя', 20, 76, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s9_body',
      '9.1. Гость обязан бережно относиться к номеру, мебели, оборудованию, отделке, инженерным системам,'
      + ' текстилю, посуду, ключам, картам доступа и иному имуществу бутик-отеля.\n'
      + '9.2. Гость несет ответственность за утрату, повреждение или загрязнение имущества бутик-отеля по своей'
      + ' вине, а также по вине приглашенных им лиц, несовершеннолетних лиц, за которых он отвечает, и иных лиц,'
      + ' которым он предоставил доступ в номер.\n'
      + '9.3. Размер ущерба определяется на основании акта, фото- и видеоматериалов при их наличии, объяснений'
      + ' сторон, стоимости восстановления, чистки, ремонта, замены имущества и действующего прейскуранта'
      + ' Исполнителя.\n'
      + '9.4. Отказ Гостя от подписания акта не освобождает его от обязанности возместить ущерб. В таком случае'
      + ' Исполнитель вправе составить односторонний акт с участием не менее двух представителей Исполнителя либо'
      + ' с использованием фото- и видеофиксации.\n'
      + '9.5. При утрате ключа / карты доступа, повреждении замка, мебели, техники, сантехники, текстиля, посуды'
      + ' или иного имущества Гость возмещает стоимость восстановления или замены по фактическим расходам'
      + ' Исполнителя либо по утвержденному прейскуранту.',
      20, 82, 173, 42,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §10 Ответственность Исполнителя и хранение вещей
    st('bm_s10_title', '10. Ответственность Исполнителя и хранение вещей', 20, 127, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s10_body',
      '10.1. Исполнитель оказывает услуги в соответствии с законодательством РФ, настоящим договором,'
      + ' подтверждением бронирования, выбранным тарифом и Правилами проживания бутик-отеля.\n'
      + '10.2. Исполнитель отвечает за сохранность вещей Гостя в случаях и пределах, предусмотренных'
      + ' законодательством РФ.\n'
      + '10.3. Денежные средства, документы, ювелирные изделия, электронная техника и иные ценные вещи'
      + ' рекомендуется сдавать администратору на хранение, если такая услуга доступна, либо хранить с принятием'
      + ' разумных мер предосторожности.\n'
      + '10.4. О пропаже, повреждении или утрате вещей Гость обязан незамедлительно сообщить администратору для'
      + ' фиксации обстоятельств, составления акта и проверки доступных материалов, включая записи камер при'
      + ' их наличии.\n'
      + '10.5. Исполнитель не отвечает за убытки Гостя, возникшие вследствие нарушения Гостем договора, Правил'
      + ' проживания, требований безопасности, правил пользования имуществом, а также вследствие действий третьих'
      + ' лиц, если ответственность Исполнителя не установлена законодательством РФ.\n'
      + '10.6. Исполнитель не несет ответственность за временное отсутствие или перебои в работе внешних'
      + ' коммунальных, телекоммуникационных, транспортных и иных служб, если такие перебои возникли не по вине'
      + ' Исполнителя. При этом Исполнитель принимает разумные меры для уменьшения неудобств Гостя в пределах'
      + ' технической и организационной возможности.\n'
      + '10.7. Исполнитель не является перевозчиком, туроператором, транспортной компанией или службой трансфера,'
      + ' если соответствующая услуга не оформлена отдельным договором или заказом. Информация о погоде, дороге,'
      + ' трансфере, такси, экскурсиях и маршрутах предоставляется как справочная поддержка.',
      20, 133, 173, 59,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
  ];
...
function createCcContractPage4Schemas() {
  return [
    // §11 Односторонний отказ и прекращение проживания
    st('bm_s11_title', '11. Односторонний отказ и прекращение проживания', 20, 8, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s11_body',
      '11.1. Гость вправе отказаться от исполнения договора в любое время при условии оплаты фактически'
      + ' оказанных услуг и фактически понесенных Исполнителем расходов, связанных с исполнением договора.\n'
      + '11.2. Исполнитель вправе отказаться от исполнения договора в случаях и порядке, предусмотренных'
      + ' законодательством РФ.\n'
      + '11.3. При грубом нарушении Гостем Правил проживания, требований пожарной безопасности, общественного'
      + ' порядка, прав других гостей, создании угрозы жизни, здоровью или имуществу, а также при отказе'
      + ' прекратить нарушение после требования администрации Исполнитель вправе потребовать прекращения'
      + ' нарушения, возмещения ущерба и освобождения номера в порядке, предусмотренном законодательством РФ.\n'
      + '11.4. Прекращение проживания по причинам, связанным с виновными действиями Гостя, не освобождает Гостя'
      + ' от оплаты фактически оказанных услуг, фактически понесенных Исполнителем расходов и возмещения'
      + ' причиненного ущерба.',
      20, 14, 173, 34,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §12 Персональные данные и электронные коммуникации
    st('bm_s12_title', '12. Персональные данные и электронные коммуникации', 20, 51, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s12_body',
      '12.1. Подписывая настоящий договор, направляя заявку на бронирование, оплачивая услуги или заселяясь в'
      + ' бутик-отель, Гость подтверждает предоставление персональных данных, необходимых для заключения и'
      + ' исполнения договора, регистрации проживания, бухгалтерского учета, исполнения требований'
      + ' законодательства РФ и связи с Гостем по вопросам проживания.\n'
      + '12.2. Исполнитель обрабатывает персональные данные Гостя в объеме и целях, необходимых для исполнения'
      + ' договора и требований законодательства РФ.\n'
      + '12.3. Юридически значимые уведомления, подтверждения бронирования, счета, сообщения об изменении или'
      + ' отмене бронирования могут направляться по адресу электронной почты, номеру телефона, через сайт, систему'
      + ' бронирования или иной канал связи, указанный Гостем при бронировании.\n'
      + '12.4. Стороны признают переписку, подтверждения бронирования, счета, платежные документы, электронные'
      + ' сообщения и иные материалы, позволяющие установить отправителя, содержание и дату сообщения,'
      + ' допустимыми доказательствами условий бронирования, оплаты и исполнения договора.',
      20, 57, 173, 31,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §13 Форс-мажор, погодные условия и транспортная доступность
    st('bm_s13_title', '13. Форс-мажор, погодные условия и транспортная доступность', 20, 91, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s13_body',
      '13.1. Стороны освобождаются от ответственности за частичное или полное неисполнение обязательств, если оно'
      + ' явилось следствием чрезвычайных и непредотвратимых обстоятельств, включая стихийные бедствия, опасные'
      + ' погодные условия, аварийные отключения внешних сетей, ограничения проезда, закрытие или перекрытие'
      + ' автомобильных дорог, переметы, штормовые предупреждения, действия органов власти и иные обстоятельства'
      + ' вне разумного контроля Сторон.\n'
      + '13.2. Сторона, для которой создалась невозможность исполнения обязательств, уведомляет другую Сторону в'
      + ' разумный срок и принимает зависящие от нее меры для уменьшения возможных последствий.\n'
      + '13.3. Гость уведомлен, что бутик-отель расположен в с. Териберка, где транспортная доступность может'
      + ' зависеть от погодных условий, состояния автомобильных дорог, работы внешних транспортных служб,'
      + ' ограничений движения и решений уполномоченных органов.\n'
      + '13.4. Исполнитель не гарантирует возможность прибытия Гостя в бутик-отель или обратного выезда Гостя из'
      + ' с. Териберка в конкретную дату и время, если препятствия возникли вследствие погодных условий,'
      + ' ограничений проезда, перекрытия дорог, отсутствия транспорта, действий третьих лиц или обстоятельств,'
      + ' не зависящих от Исполнителя.\n'
      + '13.5. Расходы Гостя, связанные с изменением маршрута, ожиданием транспорта, продлением поездки,'
      + ' изменением авиабилетов, трансфера, экскурсий или иных услуг третьих лиц, не возмещаются Исполнителем,'
      + ' если такие расходы возникли не по вине Исполнителя.\n'
      + '13.6. Перекрытие дороги, неблагоприятные погодные условия, отсутствие транспортного сообщения или'
      + ' невозможность Гостя своевременно покинуть бутик-отель не являются основанием для бесплатного продления'
      + ' проживания, если иное отдельно не согласовано с администрацией. Продление проживания в такой ситуации'
      + ' допускается только при наличии свободных номеров, технической и организационной возможности бутик-отеля'
      + ' и с оплатой по действующему тарифу бутик-отеля.',
      20, 97, 173, 56,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),

    // §14 Порядок разрешения споров
    st('bm_s14_title', '14. Порядок разрешения споров', 20, 156, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s14_body',
      '14.1. Все спорные вопросы Стороны стремятся урегулировать путем переговоров и обмена письменными'
      + ' обращениями.\n'
      + '14.2. Претензии Гостя рассматриваются Исполнителем в порядке и сроки, предусмотренные'
      + ' законодательством РФ.\n'
      + '14.3. При невозможности урегулировать спор путем переговоров спор подлежит рассмотрению в судебном'
    ...
    st('bm_s15_title', '15. Заключительные положения', 20, 185, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st(
      'bm_s15_body',
      '15.1. Настоящий договор вступает в силу с момента его подписания Сторонами, направления Исполнителем'
      + ' подтверждения бронирования, внесения Гостем оплаты / предоплаты или фактического заселения Гостя —'
      + ' в зависимости от того, какое событие наступило раньше.\n'
      + '15.2. Договор действует до полного исполнения Сторонами обязательств, включая оплату услуг, возврат'
      + ' ключей, возмещение ущерба при его наличии и проведение окончательных расчетов.\n'
      + '15.3. Правила проживания бутик-отеля, подтверждение бронирования, действующий тариф, счет, прейскурант'
      + ' дополнительных услуг и акт о причинении ущерба при его составлении являются документами, связанными с'
      + ' исполнением настоящего договора.\n'
      + '15.4. Если отдельное положение договора будет признано недействительным, это не влияет на'
      + ' действительность остальных положений договора.\n'
      + '15.5. Гость подтверждает, что до заключения договора получил необходимую информацию об Исполнителе,'
      + ' бутик-отеле, условиях проживания, цене, порядке оплаты, условиях бронирования и отмены, Правилах'
      + ' проживания бутик-отеля, а также о правах, обязанностях и ограничениях, связанных с проживанием.\n'
      + '15.6. Подписание настоящего договора Гостем означает согласие Гостя с Правилами проживания бутик-отеля'
      + ' «Большая Медведица». Гость подтверждает, что обязуется соблюдать Правила проживания лично, а также'
      + ' обеспечить их соблюдение зарегистрированными гостями, несовершеннолетними лицами, сопровождаемыми'
      + ' Гостем, посетителями и иными лицами, допущенными Гостем на территорию бутик-отеля или в номер.',
      20, 191, 173, 51,
      { fontSize: 7.15, lineHeight: 1.08 },
    ),
  ];
...
function createCcContractPage5Schemas() {
  return [
    // §16 Реквизиты и подписи Сторон
    // cc_section_6_title, cc_executor_party_title, cc_executor_requisites,
    // cc_client_party_title, cc_client_requisites positions are controlled by CC_LAYOUT_OVERRIDES.
    st('cc_section_6_title', '16. Реквизиты и подписи Сторон', 20, 8, 173, 5, {
      fontName: 'NotoSerifBold', alignment: 'center',
    }),
    st('cc_executor_party_title', 'Исполнитель\nООО «Золото Арктики»', 20, 15, 88, 8, {
      fontSize: 8.2,
      alignment: 'center',
      fontName: 'NotoSerifBold',
      lineHeight: 1.08,
    ...
    st(
      'cc_executor_requisites',
      'ИНН: 5105013870   КПП: 510501001\n'
      + 'ОГРН: 1215100000158\n'
      + 'Юр. адрес: 184433, Мурманская обл.,\n'
      + 'Печенгский р-н, г. Заполярный,\n'
      + 'ул. Ленина, д. 1А, помещение 34\n'
    ...
    // cc_client_party_title text is overridden by CC_LAYOUT_OVERRIDES to 'Гость\n{client_name}'
    mvt('cc_client_party_title', 'Гость\n{client_name}', ['client_name'], 112, 15, 81, 8, {
      fontSize: 8.2,
      fontName: 'NotoSerifBold',
      lineHeight: 1.12,
      alignment: 'center',
    ...
    imageField('cc_executor_signature_image', 29, 89, 34, 20),
    imageField('cc_company_stamp_image', 21, 81, 44, 44, { opacity: 0.68 }),
    l('cc_executor_sign_line', 44, 109, 37, 0.1),
    st('cc_executor_sign_name', '/Е. А. Сташ/', 74, 105, 24, 5, { fontSize: 7.8 }),
    l('cc_client_sign_line', 115, 109, 37, 0.1),
    t('cc_client_sign_name', 155, 105, 32, 5, {
      fontSize: 7.8,
      fontName: 'NotoSerifBold',
    }),
  ];
...
function createDefaultChungaChangaContractOnlyPdfmeTemplate(): Template {
  return {
    ...
    schemas: [
      ...
      applyStandardLayoutOverrides(createCcContractPage3Schemas()),
      applyStandardLayoutOverrides(createCcContractPage4Schemas()),
      createCcContractPage5Schemas(),
    ],
  };
}

function isInvoiceOrActPage(page: Template['schemas'][number]): boolean {
  return page.some(schema => {
    const name = String((schema as Record<string, unknown>).name || '');
    return name === 'invoice_header' || name === 'act_header';
  });
}
...
export function createChungaChangaPackagePdfmeTemplate(
  ...
  let contractPages = (contractTemplate.schemas || [])
    .filter(page => !isInvoiceOrActPage(page))
    .map(page => normalizeChungaChangaContractPage(page));

  if (contractPages.length === 0) {
    contractPages = createDefaultChungaChangaContractOnlyPdfmeTemplate().schemas
      .map(page => normalizeChungaChangaContractPage(page));
  }
...
export function hasChungaChangaPackagePages(template: Template) {
  ...
  if (schemas.length < 3) return false;
  const penultimate = schemas[schemas.length - 2];
  const last = schemas[schemas.length - 1];
  return Boolean(
    penultimate?.some(schema => String((schema as Record<string, unknown>).name) === 'invoice_header')
    && last?.some(schema => String((schema as Record<string, unknown>).name) === 'act_header')
  );
}
...
export function ensureChungaChangaPackagePdfmeTemplate(
  ...
  const schemas = template.schemas;
  const contractPages = schemas.slice(0, schemas.length - 2);
  const invoiceActPages = schemas.slice(schemas.length - 2);

  return {
    ...template,
    basePdf: { width: 210, height: 297, padding: [5, 15, 5, 20] },
    schemas: [
      ...contractPages.map(page => normalizeChungaChangaContractPage(page)),
      ...invoiceActPages.map(page => normalizeDocumentSignatureImagesAndLayers(page)),
    ],
  };
}

export function createGolubayaBukhtaPackagePdfmeTemplate(
  contractTemplate: Template = createDefaultGolubayaBukhtaContractOnlyPdfmeTemplate(),
  invoiceActTemplate: Template = createDefaultInvoicePdfmeTemplate(),
): Template {
  const invoiceAct = ensureInvoiceActPage(invoiceActTemplate);
  ...
  while (contractPages.length < 2) {
    const fallback = createDefaultGolubayaBukhtaContractOnlyPdfmeTemplate().schemas[contractPages.length];
